		...
		$result1=mysqli_query($con,"select * from package where type='general'");
		 if(mysqli_num_rows($result1)>0)
		   ...
		   while($row1=mysqli_fetch_array($result1))
			...
			$id=$row1['id'];
			echo "<tr>
        			     <th background='data:image/jpeg;base64,".base64_encode($img)."' style='width:50%; height:50%'><h1 class='circle-btn' style='width:25%; float:right'>Book now Starting from ".$row1['price_deluxe']." per person<br>".$row1['days']."</h1><br>
